...
REM Crear una tarea programada para verificar cada 5 minutos que la tarea principal esté ejecutándose
schtasks /create /tn "MonitorRunScriptLaba" /tr "cmd /c tasklist /fi \"imagename eq wscript.exe\" | findstr /i run_script_hidden.vbs || schtasks /run /tn RunScriptLaba_OnLogon" /sc minute /mo 5 /f /rl highest
